    ...
    for os_str in os_list:
        ...
        Report.set_events_data(additional_parameters=None,
                               ...
                               events_list=[(["tap\_ buy\_%", "tap\_buy\_banner%"], "%success%"),
                                            (["enter_mishki_from_Mult", "enter_mashka_from_JamDay"],)])
        ...
        while Report.get_next_event():
            ...
            if Report.current_user.publisher != "Branch" and not source and Report.current_user.source == OS.get_source(
                    os):
                if Report.current_event.obj_name == "enter_mishki_from_Mult":
                    publisher = "Branch"
                    source = "Additional Mult"
                elif Report.current_event.obj_name == "enter_mishki_from_JamDay":
                    publisher = "Branch"
                    source = "Additional JamDay"
            ...
            if day_in_game > previous_day_in_game:
                for day in range(previous_day_in_game, day_in_game):
                    if day > len(user_accumulative.keys()):
                        break
                    user_accumulative[str(day) + "d"] = ltv
            ...
            if Report.current_event.obj_name not in ("enter_mashka_from_JamDay", "enter_mishki_from_Mult"):
                ...
                if publisher != "Organic":
                    revenue += Report.current_event.price
            # Переходим на следующий день
            previous_day_in_game = day_in_game

        flush_user_data()
        print(OS.get_os_string(os), revenue)
        # РАСЧЕТЫ И ВЫВОД
        ...
        for publisher in sources.keys():
            ...
            y = approximator(np.arange(0, days_since_install + 31, 1))
            plt.plot(range(len(y_real_arpu)), y_real_arpu, '*', color="green", label="known")
            plt.plot(np.arange(0, days_since_install + 31, 1), y, '--', color="red", label="approximate")
            plt.legend()
